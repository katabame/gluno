...
import { dangerousAPI, log } from "./lib/logger.js";
...
const browserPaths = ({
  ...
  linux: { // these should be in path so just use the name of the binary
    ...
    edge_dev: [
      "microsoft-edge-dev",
      "microsoft-edge-browser-dev",
      "microsoft-edge-dev-browser",
    ],
    edge_canary: [
      "microsoft-edge-canary",
      "microsoft-edge-browser-canary",
      "microsoft-edge-canary-browser",
    ],
  ...
  darwin: {
    ...
    edge_beta:
      "/Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge Beta",
    edge_dev:
      "/Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge Dev",
    edge_canary:
      "/Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge Canary",
...
const getBrowserPath = async (browser) => {
  for (
    const path of Array.isArray(browserPaths[browser])
      ? browserPaths[browser]
      : [browserPaths[browser]]
  ) {
...
const getFriendlyName = (whichBrowser) =>
...
const getBrowserType = (name) => { // todo: not need this
  if (
    ...
    ["librewolf", "waterfox"].includes(name)
  ) return "firefox";
...
const generatePort =
  () => (Math.floor(Math.random() * (portRange[1] - portRange[0] + 1)) +
    portRange[0]);
...
const startBrowser = async (
  ...
  const localUrl = browserType === "firefox"
    ? `http://localhost:${generatePort()}`
    : "https://gluon.local";
  ...
  if (openingLocal && browserType === "firefox") {
    closeHandlers.push(
      await LocalHTTP({ url: localUrl, basePath, csp: localCSP }),
    );
  }
...
const getParentDir = () =>
  dirname(
    fileURLToPath(
      (new Error()).stack.split("\n")[3].slice(7).trim().split(":").slice(0, -2)
        .join(":"),
    ),
  );
...
const checkForDangerousOptions = ({ allowHTTP, allowNavigation }) => {
  ...
  if (allowNavigation === true) {
    dangerousAPI("Gluon.open", "allowNavigation", "true");
  }
};
