...
const presets = { // Presets from OpenAsar
  "base":
    "--autoplay-policy=no-user-gesture-required --disable-features=WinRetrieveSuggestionsOnlyOnDemand,HardwareMediaKeyHandling,MediaSessionService", // Base
  "perf":
  ...
  return await StartBrowser(
    ...
    [
      ...
      Array.isArray(extensions) && extensions.length > 0
        ? `--load-extension=${(await Promise.all(extensions)).flat().join(",")}`
        : "",
      ...`--new-window --autoplay-policy=no-user-gesture-required --disable-translate --disable-sync --no-first-run --no-default-browser-check --disable-component-extensions-with-background-pages --disable-default-apps --disable-breakpad --disable-crashpad --disable-background-networking --disable-domain-reliability --disable-component-update --disable-sync --disable-features=AutofillServerCommunication --in-process-gpu ${presets.perf}`
        .split(" "),
    ],
    "stdio",
    extra,
