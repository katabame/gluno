...
export default (CDP, evaluate, { browserType, dataPath }) => {
  if (browserType !== "chromium") { // current implementation is for chromium-based only
    const warning = () =>
      log(
        `Warning: V8 Cache API is only for Chromium (running on ${browserType})`,
      );
  ...
  const build = async (
    ...
    const _cache = async () => {
      ...
      if (reload) {
        if (finishOnLoad) {
          CDP.on("Page.frameStoppedLoading", async () => {
    ...
    log(
      `v8Cache: cached ${cache.length}/${urls.length} scripts in ${
    ...
    log(
      `v8Cache: saved to .../${path.split(sep).slice(-3).join("/")} (${
        (Buffer.byteLength(raw, "utf8") / 1024 / 1024).toFixed(2)
      }MB)`,
    );
  };
  ...
  const load = async (path = getDefaultPath()) => {
    ...
    log(
      `v8Cache: loaded ${cache.length} scripts in ${
        (performance.now() - startTime).toFixed(2)
      }ms`,
    );
